...
sys.path.append(
    os.path.join(os.path.join(os.path.dirname(os.path.dirname(__file__)), "src"))
)
import unittest
import core
...
class CircuitInstantiation(TestCaseAppended):
...
class SeriesRLC(TestCaseAppended):
    ...
    def parameters(self, R, L, C):
        circuit = core.Network([core.C(0, 1, C), core.L(1, 2, L), core.R(0, 2, R)])
    ...
    def test_frequency(self):
        ...
        cpx_w = (1j * C * R + np.sqrt(4 * C * L - C ** 2 * R ** 2)) / (2.0 * C * L)
...
class Transmon(TestCaseAppended):
    ...
    def test_double_series_capacitor(self):
        ...
        self.assertArrayRelativelyClose(
            [e ** 2 / 2.0 / C / h, 1 / (np.sqrt(C * Lj) * 2.0 * pi)], [A[0], f[0]]
        )
...
class TestNetworkAnalysis(TestCaseAppended):
    ...
    def test_transfer_voltage_divider(self):
        ...
        net = core._Network(
            [core.R(0, 1, "Z2"), core.R(1, 2, "Z1"), core.R(2, 0, "Zg"),]
        )
        ...
        self.assertRelativelyClose(
            transfer.evalf(subs=test_parameters), transfer_theory(**test_parameters)
        )
    ...
    def test_open_or_series_check(self):
        with self.assertRaises(ValueError):
            net = core._Network(
                [core.R(0, 1, "Z"), core.C(1, 2, "Z"), core.J(2, 3, "Z"),]
            )
    ...
    def test_connectivity_check_single_element_not_connected(self):
        with self.assertRaises(ValueError):
            net = core._Network(
                [core.R(0, 1, "Z"), core.C(1, 2, "Z"), core.J(3, 4, "Z"),]
            )
