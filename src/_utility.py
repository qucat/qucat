            ...
            for kw, arg in kwargs.items():
                if kw not in exclude:
                    try:
                        iter(arg)
                        raise ValueError(
                            "No iterables are allowed, use a single value for %s" % kw
                        )
                    except TypeError:
                        # not an iterable
                        pass
                    ...
                    try:
                        ...
                        if len(iterables) > 0:
                            ...
                            if np.array(arg).shape != first_iterable.shape:
                                raise ValueError(
                                    "Keyword arguments have incompatible shapes: %s %s and %s %s"
                                    % (
                                        list(iterables)[0],
                ...
                for index, _ in np.ndenumerate(first_iterable):
                    ...
                    if i == 0:
                        try:
                            ...
                            to_return = np.empty(
                                (*first_iterable.shape, *to_return_single.shape),
                                dtype=np.complex128,
                            )
                        except TypeError:
                            # not an iterable
                            to_return = np.empty(
                                first_iterable.shape, dtype=np.complex128
                            )
                        i += 1
...
def get_float_part(v, exponent_3, maximum_info):
    ...
    if maximum_info == False:
        ...
        if (
            (v - float("%2.6e" % v) != 0 and float_part >= 100.0)
            or (v - float("%1.6e" % v) != 0 and 100 > float_part >= 10.0)
            or (v - float("%.6e" % v) != 0 and float_part < 10.0)
        ):
...
def pretty_value(
    v, is_complex=True, use_power_10=False, use_unicode=True, maximum_info=False
):
    ...
    if is_complex:
        ...
        exponent = exponent_3_to_string(
            max(np.absolute(np.real(v)), np.absolute(np.imag(v))),
            exp3,
            use_power_10,
            use_unicode,
        )
